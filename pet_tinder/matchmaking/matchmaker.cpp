...
void Matchmaker::refreshPetList(){
    ...
    for(int i = currentUser->dislikedPetIds.size() - 1; i < (int)discardedPetIds.size() - 1; i++){

        discardedPetIds.push_back(currentUser->likedPetIds.at(i));
    }


    //cout << DM->getNumPets() << endl;
    //sorts through all pets and adds to a vector if they are not liked/disliked
    for(int j = 0; j < DM->getNumPets() - 1; j++){
        int flag = 0;
        for(int k = 0; k < (int)discardedPetIds.size() - 1; k++){
